...
def get_pcb_bord(filters):
	...
	for i in time_logs: 
		row_dict = {}
		all_parent = frappe.get_all("Job Card Time Log", filters = {'pcb_qr_code':i.pcb_qr_code}, fields = ['product_type','parent','pcb_qr_code'] , distinct="pcb_qr_code")
		honeywell_qr = None
		if i.pcb_qr_code:
			honeywell_qr=frappe.get_value("Job Card Time Log",{'pcb_qr_code':i.pcb_qr_code , 'parent':"PO-JOB00266"},'honeywell_qr')
		name_dict = {"pcb_code" :str(i.pcb_qr_code)}
		honeywell_qr_dict = {"honeywell_qr": str(honeywell_qr)}
		for j in all_parent:
			each_parent_dict = {str(j.parent) : str(j.product_type)}
			row_dict = {**row_dict ,**each_parent_dict}
		row_dict = {**row_dict , **name_dict,  **honeywell_qr_dict}
...
def create_operation_columns(filters):
	...
	coloumns.append(honeywell_qr_field)
